...
func (m MyImplTwo) buildTree(preorder []int, inorder []int) *TreeNode {
...
func (m MyImplTwo) smallestDifference(a []int, b []int) int {
	...
	i, j := 0, 0
	for i < len(a) && j < len(b) {
		res = int(math.Min(float64(res), math.Abs(float64(a[i]-b[j]))))
		if a[i] < b[j] {
			i++
		} else if a[i] > b[j] {
			j++
...
func (m MyImplTwo) longestValidParentheses(s string) int {
	res := 0
	cnt := 0
	for i := 0; i < len(s); i++ {
		if s[i] == '(' {
